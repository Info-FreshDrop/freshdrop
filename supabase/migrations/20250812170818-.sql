-- Fix Critical Security Issues: Operator Applications and Invites Tables

-- 1. Remove overly permissive policy from operator_applications
DROP POLICY IF EXISTS "Anyone can create applications" ON public.operator_applications;

-- 2. Create secure policies for operator_applications 
...
-- 3. Fix operator_invites table - restrict token-based access
...
FOR SELECT 
USING (
  -- Only allow access if signup_token is provided in query and invite is not expired
  signup_expires_at > now() AND is_used = false
);

-- 4. Enhance profile security - ensure users can only access their own data
