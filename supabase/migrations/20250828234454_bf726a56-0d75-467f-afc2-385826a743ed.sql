-- Create a trigger that runs when an operator application status is changed to 'approved'
-- This will call the operator approval notification edge function

-- Drop existing trigger if it exists
DROP TRIGGER IF EXISTS trigger_operator_approval ON operator_applications;

-- Drop existing function if it exists  
DROP FUNCTION IF EXISTS handle_operator_approval();
