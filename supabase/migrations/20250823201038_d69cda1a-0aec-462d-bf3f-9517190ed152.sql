...
BEGIN
  ...
  LOOP
    ...
    result_message := 'Calculated earnings: $' || (total_earnings_cents::decimal / 100) || ' (revenue: $' || (revenue_share_cents::decimal / 100) || ' + tips: $' || (total_tips_cents::decimal / 100) || ')';
    RETURN NEXT;
  END LOOP;
  
  -- If no orders processed, return summary
  IF result_count = 0 THEN
    processed_order_id := NULL;
    result_message := 'No completed orders found that need earnings calculation';
    RETURN NEXT;
  END IF;
