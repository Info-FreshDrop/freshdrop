-- Update insert_order_message function to properly trigger email notifications
...
BEGIN
  ...
  -- Call notification edge function for message notifications
  -- Only send notification to the recipient (not the sender)
  PERFORM net.http_post(
    url := 'https://sbilxpqluuvxhmgfpggx.supabase.co/functions/v1/send-order-notifications',
    ...
    body := jsonb_build_object(
      ...
      'customer_id', v_recipient_user_id,
      'operator_id', CASE WHEN v_sender_is_customer THEN v_recipient_user_id ELSE v_sender_id END,
      'order_id', p_order_id,
      'subject', CASE WHEN v_sender_is_customer THEN 'New message from customer' ELSE 'New message from your operator' END,
      'message', p_message,
